...
class HeadOfUnitRepositories(Base[HeadOfUnit]):
    ...
    def get_commissioners_under_jurisdiction(
        self, db: Session, jurisdiction_id: str
    ) -> List[CommissionerProfile]:
        court_ids = (
            db.query(Court.id).filter(Court.jurisdiction_id == jurisdiction_id).all()
        )
        court_ids = [
            court_id[0] for court_id in court_ids
        ]  # Extract court IDs from the result
        commissioner_profiles = (
            db.query(CommissionerProfile)
            .filter(CommissionerProfile.court_id.in_(court_ids))
            .all()
        )

        return commissioner_profiles

    def get_courts_under_jurisdiction(
        self, db: Session, jurisdiction_id: str
    ) -> List[CommissionerProfile]:
